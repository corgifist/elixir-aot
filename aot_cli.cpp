...
ExObject ExModule_Checker_is_true_Clause143424();
ExObject ExModule_Checker_is_true_Clause375334();
...
ExObject ExRemote_Checker_is_true(ExObject arguments) {
	...
	if (ExMatch_tryMatch(EX_LIST({EX_VAR("x")}), arguments)) {
		if (IS_TRUE(EX_ENVIRONMENT.get("x"))) {
			ExObject result = ExModule_Checker_is_true_Clause143424();
			EX_ENVIRONMENT.pop();
			return result;
		};
	}
	...
	if (ExMatch_tryMatch(EX_LIST({EX_VAR("x")}), arguments)) {
		if (IS_TRUE(EX_ATOM("true"))) {
			ExObject result = ExModule_Checker_is_true_Clause375334();
			EX_ENVIRONMENT.pop();
			return result;
		};
	}
